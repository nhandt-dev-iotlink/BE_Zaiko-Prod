...
public interface CustomerDeliveryDestRepository extends BaseRepository<CustomerDeliveryDestEntity, Integer> {
    @Query(value = "SELECT new org.api.dto.CustomerDeliveryDestDto(a.deliveryDestinationId, a.destinationCode, " +
            ...
            "a.address2, a.address3, a.address4, a.companyId, a.customerId, a.isCustomer, a.leadTime) FROM CustomerDeliveryDestEntity a WHERE a.delFlg = '0' AND (:deliveryName = '' OR a.destinationCode LIKE CONCAT('%', :deliveryName, '%') OR a.departmentName LIKE CONCAT('%', :deliveryName, '%')) ORDER BY a.destinationCode")
    List<CustomerDeliveryDestDto> getAll(@Param("deliveryName") String deliveryName);

    @Query(value = "SELECT new org.api.dto.CustomerDeliveryDestDto(a.deliveryDestinationId, a.destinationCode, " +
            "a.departmentName, a.phoneNumber, a.faxNumber, a.postCode, a.routeCode, a.courseCode, a.address1, " +
            "a.address2, a.address3, a.address4, a.companyId, a.customerId, a.isCustomer, a.leadTime) FROM CustomerDeliveryDestEntity a WHERE a.delFlg = '0' AND a.destinationCode = :code")
